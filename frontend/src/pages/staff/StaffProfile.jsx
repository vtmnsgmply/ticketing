...
import { changeStaffPassword, getStaffProfile, updateStaffProfile } from '../../services/staffService'
...
export default function StaffProfile() {
  ...
  useEffect(() => {
    getStaffProfile().then((user) => setProfile({
      name: user.name ?? '',
      email: user.email ?? '',
      phone: user.phone ?? '',
  ...
  async function submitProfile(event) {
    ...
    setErrors({})
    try {
      const user = await updateStaffProfile(profile)
  ...
  return (
    <AppShell width="standard">
      <PageHeader eyebrow="Staff Workspace" title="Profile" />
